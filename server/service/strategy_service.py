...
from server.service.ticker_service import get_stock_data, get_stock_list
import pandas as pd
...
async def generate_super_trend_with_ma_data(): 
    ...
    all_keys = [x['instrument_key'] for x in data]
    key_to_name = {x['instrument_key']:x['name'] for x in data}
